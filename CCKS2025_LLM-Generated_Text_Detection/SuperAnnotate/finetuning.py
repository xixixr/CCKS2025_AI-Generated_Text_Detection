...
import os
os.sys.path.append("/data/workspace/xiarui/tianchi_LMTextDetect/generated_text_detector-1.1.0")
...
def train_model(model, train_loader, val_loader, optimizer, device, epochs, accumulation_steps, save_dir, writer):
    ...
    for epoch in range(epochs):
        ...
        for batch in progress_bar:
            ...
            with autocast():
                ...
                loss = torch.nn.MSELoss()(logits.view(-1), labels)
                # 梯度累积做平均
...
def main():
    args = parse_args()
    log_dir = "/data/workspace/xiarui/tianchi_LMTextDetect/CCKS2025_LLM-Generated_Text_Detection/output/SuperAnnotate/logs/runs"
    writer = SummaryWriter(log_dir=log_dir)  # TensorBoard日志目录
    mode_save_dir = "/data/workspace/xiarui/tianchi_LMTextDetect/CCKS2025_LLM-Generated_Text_Detection/output/SuperAnnotate"
    # 检查并创建保存目录
    ...
    if torch.cuda.is_available():
        ...
        if args.gpu_num > 1:
            ...
            device_ids = list([1,2,3])  # 使用指定GPU
            print(f"Using {len(device_ids)} GPUs!")
            model = torch.nn.DataParallel(model, device_ids=device_ids)
            device = torch.device(f"cuda:{device_ids[0]}")
        else:
            device_ids = list([1])  # 使用指定GPU
    ...
    model.to(device)
    # 读取数据文件
    train_file = "/data/workspace/xiarui/tianchi_LMTextDetect/CCKS2025_LLM-Generated_Text_Detection/dataset/train.jsonl"
    ...
    # 开始训练
    train_model(model, full_loader, val_loader, optimizer, device, args.epochs, args.accumulation_steps, mode_save_dir, writer)
